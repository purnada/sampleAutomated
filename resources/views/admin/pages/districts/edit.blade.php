                    ...
                    <form method="POST" action="{{ route('admin.districts.update', $district->id) }}"
                        ...
                        <div class="grid lg:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <select class="my-auto ti-form-input @error('province_id') border-red-500 @enderror"
                                    name="province_id">
                                    @foreach ($provinces as $title => $id)
                                        <option value="{{ $id }}" @selected($id == old('province_id', $district->province_id))>{{ $title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="title" type="text" name="title"
                                    class="my-auto ti-form-input @error('title') border-red-500 @enderror"
                                    placeholder="Title" value="{{ old('title',$district->title) }}" />
                                @error('title')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="space-y-2">
                                ...
                                <input id="name_nep" type="text" name="name_nep"
                                    class="my-auto ti-form-input @error('name_nep') border-red-500 @enderror"
                                    placeholder="काठमाण्डोेेें" value="{{ old('name_nep',$district->name_nep) }}" />
                                @error('name_nep')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
