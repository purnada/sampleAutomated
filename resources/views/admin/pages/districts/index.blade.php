...
<div class="col-span-12">
    <div class="box">
        <div class="box-header flex justify-between">
            ...
            <a href="{{ route('admin.districts.create') }}"  class="text-xs ti-btn ti-btn-primary"> Create {{ Str::headline(request()->segment(2)) }} <i class="ti ti-send"></i></a>
            </div>
            @endcan
        </div>
                    ...
                    <tbody>
                        ...
                        @forelse($districts as $key => $district)
                            <tr id="row_{{$key}}">
                                ...
                                <td>{{ $district->name_nep }}</td>
                                <td>
                                    @can('edit district')
                                    <a href="{{ route('admin.districts.edit', $district->id) }}" class="ti-btn ti-btn-primary" title="Edit"><i class="ti ti-pencil"></i></a>
                                    @endcan
                                    @can('delete district')
                                    <button  type="button"  onclick="confirmDelete('{{ route('admin.districts.destroy', $district->id) }}', {{$key}},'{{csrf_token()}}')" class="ti-btn ti-btn-danger" title="Delete"><i class="ti ti-trash"></i></button>
                                    @endcan
                                </td>
                            </tr>
                        @empty
