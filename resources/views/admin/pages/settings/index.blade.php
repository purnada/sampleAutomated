...
<div class="col-span-12">
    <div class="box">
        <div class="box-header flex justify-between">
            ...
            @if(auth()->user()->can('create setting') || in_array('create setting', $permissions))
            <div class="block ltr:ml-auto rtl:mr-auto my-auto">
            <a href="{{ route('admin.settings.create') }}"  class="text-xs ti-btn ti-btn-primary"> Create {{ Str::headline(request()->segment(2)) }} <i class="ti ti-send"></i></a>
            </div>
            @endif
        </div>
                    ...
                    <tbody>
                        ...
                        @forelse($settings as $key => $setting)
                            <tr id="row_{{$key}}">
                                ...
                                <td>{{ $setting->title }}</td>
                                <td>
                                    @if(auth()->user()->can('edit setting') || in_array('edit setting', $permissions))
                                    <a href="{{ route('admin.settings.edit', $setting->id) }}" class="ti-btn ti-btn-primary" title="Edit"><i class="ti ti-pencil"></i></a>
                                    @endif
                                    @if(auth()->user()->can('delete setting') || in_array('delete setting', $permissions))
                                    <button  type="button"  onclick="confirmDelete('{{ route('admin.settings.destroy', $setting->id) }}', {{$key}},'{{csrf_token()}}')" class="ti-btn ti-btn-danger" title="Delete"><i class="ti ti-trash"></i></button>
                                    @endif
                                </td>
                            </tr>
                        @empty
