                ...
                <form method="POST" action="{{ route('admin.roles.update',$role->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="grid lg:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input id="name" name="name" type="text" class="my-auto ti-form-input @error('name') border-red-500 @enderror" placeholder="name"  value="{{ old('name') ?? $role->name }}" />
                            @error('name')
                        ...
                        <div class="space-y-2">
                            ...
                            <select class="ti-form-select choices-multiple-remove-button  @error('permission') border-red-500 @enderror" name="permission[]" multiple>
                                @foreach ($permissions as $permi)
                                    <option value="{{ $permi }}" @selected(in_array($permi,$role_permissions))>{{ $permi }}</option>
                                @endforeach
