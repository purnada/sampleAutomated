...
<div class="col-span-12">
    <div class="box">
        <div class="box-header flex justify-between">
            ...
            <a href="{{ route('admin.roles.create') }}"  class="text-xs ti-btn ti-btn-primary"> Create {{ Str::headline(request()->segment(2)) }} <i class="ti ti-send"></i></a>
            </div>
            @endif
        </div>
                        ...
                        @forelse($roles as $key => $role)
                            <tr id="row_{{$key}}">
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('admin.roles.edit', $role->id) }}" class="ti-btn ti-btn-primary" title="Edit"><i class="ti ti-pencil"></i></a>
                                    @endif
                                    
                                    @if(auth()->user()->can('delete role'))
                                       <button  type="button"  onclick="confirmDelete('{{ route('admin.roles.destroy', $role->id) }}', {{$key}},'{{csrf_token()}}')" class="ti-btn ti-btn-danger" title="Delete"><i class="ti ti-trash"></i></button>
                                       @endif
                                       @endif
                                </td>
                            </tr>
