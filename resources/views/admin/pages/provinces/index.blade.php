...
<div class="block justify-between page-header sm:flex">
    ...
    <ol class="flex items-center whitespace-nowrap min-w-0">
        <li class="text-sm">
            <a class="flex items-center font-semibold text-primary hover:text-primary dark:text-primary truncate" href="{{ route('admin.provinces.index') }}">
...
<div class="col-span-12">
    <div class="box">
        <div class="box-header flex justify-between">
            ...
            <a href="{{ route('admin.provinces.create') }}"  class="text-xs ti-btn ti-btn-primary"> Create {{ Str::headline(request()->segment(2)) }} <i class="ti ti-send"></i></a>
            </div>
            @endcan
        </div>
                    ...
                    <tbody>
                        ...
                        @forelse($provinces as $key => $province)
                            <tr id="row_{{$key}}">
                                ...
                                <td>
                                    @can('edit province')
                                    <a href="{{ route('admin.provinces.edit', $province->id) }}" class="ti-btn ti-btn-primary" title="Edit"><i class="ti ti-pencil"></i></a>
                                    @endcan
                                    @can('delete province')
                                    <button  type="button"  onclick="confirmDelete('{{ route('admin.provinces.destroy', $province->id) }}', {{$key}},'{{csrf_token()}}')" class="ti-btn ti-btn-danger" title="Delete"><i class="ti ti-trash"></i></button>
                                    @endcan
                                </td>
                            </tr>
                        @empty
